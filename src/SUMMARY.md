# Summary

[Introduction](./README.md)

-   [Ecosystem walkthrough](./ecosystem_walkthrough/README.md)
    -   [Booting](./ecosystem_walkthrough/booting.md)
    -   [Register access layer](./ecosystem_walkthrough/ral.md)
    -   [Hardware abstraction layer](./ecosystem_walkthrough/hal.md)
    -   [Board support package](./ecosystem_walkthrough/bsp.md)
    -   [Extra packages](./ecosystem_walkthrough/extras.md)
-   [Toolchain setup](./toolchain.md)
-   [Flashing and debugging](./flash_debug.md)
-   [External documentation](./external_docs.md)
